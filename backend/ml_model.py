...
class DrugRiskPredictor:
    ...
    def _load_model(self):
        ...
        if not os.path.exists(model_path):
            print(f"[ML] No model found at {model_path} — running in rules-only mode")
            return
        ...
        try:
            ...
            self.loaded = True
            print(f"[ML] Model loaded (accuracy: {artifacts.get('accuracy', 'N/A')})")
    ...
    def predict(self, drug_name, patient_age=50, patient_sex="F",
        ...
        if not self.loaded:
            return {"score": None, "category": None, "label": "Model not available", "confidence": 0, "available": False}
        ...
        try:
            ...
            return {"score": None, "category": None, "label": f"'{drug_name}' not in training data", "confidence": 0, "available": False}

        sex_encoded = 1 if patient_sex.upper() in ("F", "FEMALE") else 0

        features = np.array([[
            drug_encoded, patient_age, sex_encoded, patient_weight,
            reaction_count, 1.0, 1, num_concomitant_drugs,
            known_side_effects, mean_se_freq, max_se_freq,
        ]])
        ...
        try:
            ...
            return {"score": None, "category": None, "label": f"Prediction error: {e}", "confidence": 0, "available": False}
